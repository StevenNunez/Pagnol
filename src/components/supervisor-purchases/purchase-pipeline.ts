...
export type PurchaseStage = 'waiting_adc' | 'in_review' | 'approved' | 'ordered' | 'received' | 'rejected';
...
export function resolvePurchaseStage(req: PurchaseRequest): PurchaseStage {
    ...
    // pending
    return req.adcAuthorizedAt ? 'in_review' : 'waiting_adc';
}
...
export function matchesHistoryFilter(stage: PurchaseStage, filter: HistoryFilter): boolean {
    ...
    if (filter === 'in_progress') return stage === 'waiting_adc' || stage === 'in_review';
    return stage === filter;
}
